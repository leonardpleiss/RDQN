...
for init_max in [.1, 1.]:
    for stochastic in [True, False]:
        for approximator in approximators:
            for discount_factor_type in ["discounted", "flat"]:

                print("--------------------------------------------------------------------")
                print(f"stochastic{stochastic}_approximator{approximator}_initMax{init_max}_{discount_factor_type}")
                print("--------------------------------------------------------------------")
                ...
                for episode_length in np.arange(2, max_episode_length+1, 1):
                    ...
                    if discount_factor_type == "discounted":
                        discount_factor = 1. - (1/episode_length)
                    elif discount_factor_type == "flat":
                        discount_factor = 1.
                    ...
                    def get_rel_weights_(td_errors, episodes, alpha2):
                        ...
                        # max_subsequent_td = np.max(subsequent_tds)
                        max_subsequent_td = np.max(subsequent_tds) * 2
                        # print(f"{max_subsequent_td=}")
                    ...
                    for method in methods:
                        ...
                        for seed in range(iterations):
                            ...
                            # print(f"Initial Qs: {learned_q_values}")
                            ...
                            while not np.isclose(np.mean((target_q_values - learned_q_values)**2), 0, atol=tol).all():
                                ...
                                sampled_idxes.append(states[idx] * 2 + actions[idx])
                            ...
                            # unique, counts = np.unique(sampled_idxes, return_counts=True)
                            # print(f"{np.asarray((unique, counts)).T=}")
                            # print(f"{target_q_values - learned_q_values}")
                            # print(f"{correct_path=}")

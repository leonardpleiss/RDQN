...
methods = ["reaper"] # Available are "uni", "per", "reaper"
...
for approximator in approximators:
    ...
    for episode_length in np.arange(max_episode_length, max_episode_length+1):
        ...
        winning_ep_mask = np.array(episodes) == winning_episode
        ...
        def get_rel_weights(td_errors, episodes):
            ...
            rel_weight = 1 - (subsequent_tds / max_subsequent_td)
        ...
        def get_sampling_idx(method, episodes, q_values, rewards, dones, alpha, use_IS):
            ...
            if method == "uni":
                ...
                rel_weights = np.ones_like(abs_td_errors) / len(abs_td_errors)
                ...
                rel_weights = get_rel_weights(abs_td_errors, episodes)
            ...
            return idx, states[idx], actions[idx], td_errors[idx], abs_td_errors, IS_weight, sampling_probas, rel_weights
        ...
        rel_weights_traces = {i:[] for i in methods}
        argmax_tds = []
        argmax_probas = []
        ...
        for method in methods:
            ...
            for seed in range(iterations):
                ...
                # while (np.argmax(learned_q_values, axis=1) != correct_path).any(): 
                    ...
                    argmax_tds.append(np.argmax(abs(target_q_values - learned_q_values)))
                    if approximator in ["DQN", "LFA"]:
                        learned_q_values = get_all_q_values(agent, episode_length)
                        idx, state, action, td_error, abs_td_errors, IS_weight, sampling_probas, rel_weights = get_sampling_idx(method, episodes, learned_q_values, rewards, dones, alpha, use_IS)
                    ...
                    if (states[idx] == 10) & (actions[idx] == 0):
                        print(f"{target=}")

                    sampled_idxes.append(idx)
                    sampling_proba_trace.append(np.array(sampling_probas))
                    td_error_trace.append(np.array(abs_td_errors))
                    rel_weights_trace.append(np.array(rel_weights))

                    argmax_probas.append(np.argmax(sampling_probas))
                    counter += 1
                    ...
                    if counter%1000==0:
                        print(counter)
                        print(f"{abs_td_errors[winning_ep_mask]=}")
                        print(f"{rel_weights[winning_ep_mask]=}")
                        print(f"{sampling_probas[winning_ep_mask]=}")
                ...
                rel_weights_traces[method] = np.array(rel_weights_trace)
            
            print(f"{correct_path=}")
            for i in np.unique(argmax_tds):
                print(i, (argmax_tds==i).sum())

            print(np.where(winning_ep_mask))
            for i in np.unique(argmax_probas):
                print(i, states[i], actions[i], (argmax_probas==i).sum())
    ...
    if deep_analysis:
        ...
        ax.set_title(f"{approximator}_{seed}")
        for method in methods:
            unique_states = states[all_sampled_idxes[method]]
            unique_actions = actions[all_sampled_idxes[method]]

            unique_state_action_pairs = unique_states * 2 + unique_actions

            state_count = [(np.array(unique_state_action_pairs) == state_idx).sum() for state_idx in np.arange(2*episode_length)]
    ...
    for method in methods:
        # print(f"{sampling_proba_traces[method].shape=}")
        for state in range(episode_length):
            for action in range(2):
                sa_mask = (states == state) & (actions == action)
                label = f"S{state}A{action}"
                sa_pair_idx = state * 2 + action
                axs[sa_pair_idx].set_title(label)
                axs[sa_pair_idx].plot(sampling_proba_traces[method][:, sa_mask].sum(axis=1), label=method, color=colors[method])
    ...
    for method in methods:
        ...
        for state in range(episode_length):
            for action in range(2):
                ...
                sa_pair_idx = state * 2 + action
                axs[sa_pair_idx].set_title(label)
                axs[sa_pair_idx].plot(td_error_traces[method][:, sa_mask].mean(axis=1), label=method, color=colors[method])
    ...
    colors = {"reaper":"red", "per":"black", "uni":"blue"}
    fig, axs = plt.subplots(nrows=episode_length*2, ncols=1)
    fig.suptitle("Rel Weight Traces")
    ...
    for method in methods:
        ...
        for state in range(episode_length):
            for action in range(2):
                ...
                sa_pair_idx = state * 2 + action
                axs[sa_pair_idx].set_title(label)
                axs[sa_pair_idx].plot(rel_weights_traces[method][:, sa_mask].mean(axis=1), label=method, color=colors[method])
                axs[sa_pair_idx].set_ylim([-.05, 1.05])

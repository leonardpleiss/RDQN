...
if __name__ == "__main__":
    ...
    trial_name = "TESTRUN_r1r2r3_04" #"CP_50k_TPER5"
    ...
    buffer_names =  ["R_UNI_r3", "R_UNI_r2", "R_UNI_r1", "R_UNI_r3_RR", "R_UNI_r2_RR", "R_UNI_r1_RR"]
    model_names = ["RDQN"]
    iterations_per_env = int(sys.argv[4])
    starting_seed = int(sys.argv[5])
    conservative_rel_adjustments = [True, False]
        ...
        for iteration in range(starting_seed, iterations_per_env+starting_seed):
            for buffer_name in buffer_names:
                for model_name in model_names:
                    for conservative_rel_adjustment in conservative_rel_adjustments:
                        ...
                        if replay_buffer_kwargs:
                            ...
                            conservativity_label = "conserv" if conservative_rel_adjustment else "nonconserv"
                            export_suffix += f"_{int(alpha2*10)}a_{conservativity_label}"
                        ...
                        model = model_class(
                            ...
                            gamma=gamma,
                            conservative_rel_adjustment = conservative_rel_adjustment,

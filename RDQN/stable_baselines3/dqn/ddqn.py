    ...
    def train(self, gradient_steps: int, batch_size: int = 100) -> None:
        ...
        for gradient_step in range(gradient_steps):
            ...
            if self.replay_buffer_class.__name__ in ["PrioritizedReplayBuffer", "PrioritizedReplayBufferPropagating", "CustomPrioritizedReplayBufferCumSumProp", "CustomPrioritizedReplayBuffer", "CustomPrioritizedReplayBufferCumSum2", "CustomPrioritizedReplayBufferCumSum3", "CustomPrioritizedReplayBufferCumSum4", "CustomPrioritizedReplayBufferCumSum", "CustomPrioritizedReplayBufferCumSumWithTDProp"]:
                ...
                if not new_priorities.min() > 0:
                    print(f"{reward_ratios=}")
                    print(f"{new_priorities=}, {td_errors=}, {current_q_values=}, {target_q_values=}, {next_q_values=}, {replay_data.next_observations=}, {next_actions=}, {replay_data.observations=}")
                    raise ValueError("Negative or nan prios")
            
                self.replay_buffer.update_priorities(idxes=sample_idxs, priorities=new_priorities)
            
            elif self.replay_buffer_class.__name__ in ["CustomPrioritizedReplayBufferCumSum5", "CustomPrioritizedReplayBufferCumSum6", "CustomPrioritizedReplayBufferCumSum7"]:
                ...
                if not new_priorities.min() > 0:
                    print(f"{replay_data.rewards}, {reward_ratios=}, {new_priorities=}, {td_errors=}, {current_q_values=}, {target_q_values=}, {next_q_values=}, {replay_data.next_observations=}, {next_actions=}, {replay_data.observations=}")
                    raise ValueError("Negative or nan prios")
                
                self.replay_buffer.update_priorities(idxes=sample_idxs, priorities=new_priorities, reward_ratios=reward_ratios)
            
            elif self.replay_buffer_class.__name__ in ["CustomPropagatingPrioritizedReplayBuffer"]:
                td_errors = current_q_values - target_q_values
                new_priorities = np.abs(td_errors.detach().cpu().numpy().reshape(-1)) + 1e-6
                self.replay_buffer.update_priorities(idxes=sample_idxs, priorities=new_priorities, learning_rate=self.learning_rate)

            elif self.replay_buffer_class.__name__ in ["CustomPropagatingPrioritizedReplayBufferCumSum"]:

                reward_ratios = (abs(replay_data.rewards) / (1e-6 + abs(replay_data.rewards) + (1 - replay_data.dones) * self.gamma * abs(next_q_values))).detach().cpu().numpy().reshape(-1)
                td_errors = current_q_values - target_q_values
                new_priorities = np.abs(td_errors.detach().cpu().numpy().reshape(-1)) + 1e-6
                ...
                self.replay_buffer.update_priorities(idxes=sample_idxs, priorities=new_priorities, reward_ratios=reward_ratios, learning_rate=self.learning_rate)

            elif self.replay_buffer_class.__name__ in ["RaPPER"]:

                reward_ratios = (abs(replay_data.rewards) / (1e-6 + abs(replay_data.rewards) + (1 - replay_data.dones) * self.gamma * abs(next_q_values))).detach().cpu().numpy().reshape(-1)
                td_errors = current_q_values - target_q_values
                new_td_errors = np.abs(td_errors.detach().cpu().numpy().reshape(-1)) + 1e-6
                new_target_q_values = target_q_values.detach().cpu().numpy().reshape(-1)
                new_q_values = current_q_values.detach().cpu().numpy().reshape(-1)
                
                self.replay_buffer.update_priorities(idxes=sample_idxs, new_td_errors=new_td_errors, new_q_values=new_q_values, new_target_q_values=new_target_q_values, gamma=self.gamma, reward_ratios=reward_ratios)

            elif self.replay_buffer_class.__name__ in ["ReaPER"]:

                reward_ratios = (abs(replay_data.rewards) / (1e-8 + abs(replay_data.rewards) + (1 - replay_data.dones) * self.gamma * abs(next_q_values))).detach().cpu().numpy().reshape(-1)
                td_errors = current_q_values - target_q_values
                new_td_errors = np.abs(td_errors.detach().cpu().numpy().reshape(-1)) + 1e-6

                if not new_td_errors.min() > 0:
                    print(f"{replay_data.rewards}, {reward_ratios=}, {new_td_errors=}, {td_errors=}, {current_q_values=}, {target_q_values=}, {next_q_values=}, {replay_data.next_observations=}, {next_actions=}, {replay_data.observations=}")
                    raise ValueError("Negative or nan prios")

    ...
    def train(self, gradient_steps: int, batch_size: int = 100) -> None:
        ...
        for gradient_step in range(gradient_steps):
            ...
            if self.replay_buffer_class.__name__ in ["R_UNI"]:
                ...
                reliability = th.ones(replay_data.actions.shape[0], requires_grad=False, device=self.device)# .unsqueeze(1).float()
            ...
            if self.to_scale_with_reliability == "range_based_clip":
                ...
                next_q = (1 - replay_data.dones.float()) * self.gamma * next_q_values
            ...
            if self.to_scale_with_reliability == "scaled_bootstrap":
                ...
                next_ = (1 - replay_data.dones.float()) * self.gamma * next_q_values
                ...
                target_q_values_adj = replay_data.rewards + (1 - replay_data.dones.float()) * self.gamma * (bootstrapped_qval_maintenance + bootstrapped_qval_increase_clipped)
            ...
            if self.to_scale_with_reliability == "scaled_bootstrap_sub":
                ...
                target_q_values_adj = replay_data.rewards + (1 - replay_data.dones.float()) * self.gamma * next_q_values_adj
                loss = F.smooth_l1_loss(current_q_values, target_q_values_adj, reduction='none')
            ...
            if self.to_scale_with_reliability == "limit_target_mean_deviation":
                ...
                target_q_values = replay_data.rewards + (1 - replay_data.dones.float()) * self.gamma * adj_next_q_values
                loss = F.smooth_l1_loss(current_q_values, target_q_values, reduction='none') * reliability

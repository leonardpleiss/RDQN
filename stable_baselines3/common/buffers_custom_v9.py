...
class SelectiveReplayBuffer(ReplayBuffer):
    ...
    def __init__(        
        ...
        self.signal_mask_next = np.zeros((self.buffer_size, self.n_envs)).astype(bool)
    ...
    def add(
        ...
        is_signal = reward # np.logical_or(done, reward)
        self.signal_mask[self.pos] = is_signal.astype(bool) #reward > 0
        self.signal_mask_next[self.pos] = False
    ...
    def sample(self, batch_size: int, env: Optional[VecNormalize] = None) -> ReplayBufferSamples:
        ...
        signal_ratio = min(1., self.rewards.sum() / av_samples)
        self.signal_slots = int(batch_size * signal_ratio)
        noise_slots = batch_size - self.signal_slots

        if self.debug_level > 1:
            print("Sample")
            print(f"{self.signal_mask.sum()}")
        
        if (self.signal_mask.sum() < self.signal_slots):
            self.next_index()

        valid_indices_signal = np.argwhere(self.signal_mask)

        chosen_signal = valid_indices_signal[np.random.choice(len(valid_indices_signal), size=self.signal_slots, replace=False)]
        ...
        self.signal_mask_next[pre_row_idxes_signal, pre_col_idxces_signal] = True

        if (not self.full) and (self.signal_mask[self.buffer_size-1]):
            raise ValueError()

        self.signal_mask[row_idxes_signal, col_idxes_signal] = False
    ...
    def next_index(self):
        ...
        self.signal_mask = np.logical_or(self.signal_mask_next, self.signal_mask)
        self.signal_mask_next = np.zeros((self.buffer_size, self.n_envs)).astype(bool)
        
        if self.debug_level > 1:
            print(f"Next Index, {self.sample_counter}")
            print(f"{self.signal_mask.sum()}")
    ...
    def start_over(self):
        ...
        is_signal = self.rewards > 0 # np.logical_or(self.dones, self.rewards > 0)
        self.signal_mask = np.logical_or(is_signal, self.signal_mask)

        if self.debug_level > 0:
            print(f"Start over, {self.sample_counter}")
            print(f"{self.signal_mask.sum()}")
